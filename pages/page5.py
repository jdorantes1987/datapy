...
st.set_page_config(page_title='DataPy: Mikrowisp', 
                   layout='wide', 
                   page_icon=':vs:')
...
def lista_grupos_clientes(empresa):
    ...
    clientes = ClsData(empresa).clintes_search(str_search=patron, 
                                               resumir_info=True)
    ...
    clientes_grupos_sort = clientes_grupos.sort_values(by='cli_des',
                                                       ascending=True)
    clientes_grupos_sort['buscar_cliente'] = clientes_grupos_sort['co_cli'] + ' | ' +  clientes_grupos_sort['cli_des']
...
with st.expander("🔍 Grupos por cliente"):    
    grupo_select = str(st.selectbox('Elije un grupo:', 
                                    lista_grupos_clientes(empresa=company_selected), 0)).replace(' ', '')
    ...
    if company_selected == os.getenv('DB_NAME_IZQUIERDA_PROFIT'):
        grupo_filtrado['co_cli_sort'] = grupo_filtrado['co_cli'].str.split('-', expand=True)[1]
        grupo_filtrado['co_cli_sort'] = grupo_filtrado['co_cli_sort'].astype("int64")
        grupo = grupo_filtrado.sort_values(by='co_cli_sort', 
                                    ascending=False)
    else:
...
if len(st.session_state.datos_clientes_por_sinc_mkwsp) > 0:
    ...
    with st.expander("agregar"):
        st.dataframe(st.session_state.datos_clientes_por_sinc_mkwsp,
                    use_container_width=True,
                    hide_index=True)
...
datos_clientes_por_sinc_sin_clientes_por_agregar = datos_clientes_por_sinc[~datos_clientes_por_sinc['co_cli'].isin(clientes_a_ignorar)]
st.session_state.datos_clientes_por_sinc_prof = datos_clientes_por_sinc_sin_clientes_por_agregar
if len(st.session_state.datos_clientes_por_sinc_prof) > 0:
    st.write("⚡ Clientes por actualizar en Mikrowisp desde Profit")
    with st.expander("datos"):
        st.dataframe(st.session_state.datos_clientes_por_sinc_prof,
                    use_container_width=True,
                    hide_index=True)
...
st.session_state.datos_clientes_por_act_nodo = datos_clientes_nodo_por_sinc_mikrowisp_profit()
if len(st.session_state.datos_clientes_por_act_nodo) > 0:
    st.write("🔄 Clientes por actualizar nodo en Profit")
    with st.expander("datos"):
        st.dataframe(st.session_state.datos_clientes_por_act_nodo,
                    use_container_width=True,
                    hide_index=True)
