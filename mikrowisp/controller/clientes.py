...
class Gestor:
    ...
    def datos_clientes_por_registrar(self):
        ...
        return df[
            df['co_cli'].isin(cod_clientes_por_agregar)
        ]

    def __exec_script_clientes_mikrowisp(self, datos):
        #  Campos de la tabla clientes en BD Profit
        columnas = ['co_cli', 'cli_des', 'email', 'telefonos', 'rif', 'direc1', 'campo3']
    ...
    def add_notificaciones(self):
        ...
        data_combinada = merge(clientes_add, clientes_mkwps, 
                              left_on='co_cli', 
    ...
    def __cod_clientes_profit_mikrowisp_por_sinc(self):
        clientes_profit = self.clientes_profit()[['co_cli', 
                                                  'cli_des', 
                                                  'direc1', 
                                                  'telefonos', 
                                                  'email']].replace(nan, 
        ...
        clientes_profit_s = clientes_profit.sort_values(by=['co_cli'], 
                                                        ascending=[True]
                                            )
        clientes_mkwsp = self.clientes_mikrowisp()[['codigo_cliente', 'nombre', 'direccion_principal', 'movil', 'correo']]
        clientes_mkwsp.rename(columns={'codigo_cliente': 'co_cli', 
                                    'nombre': 'cli_des', 
        ...
        clientes_mkwsp_s = clientes_mkwsp.sort_values(by=['co_cli'], 
                                                    ascending=[True]
                                        )
        # Hace una combinación para identificar los datos comunes
        clientes_iguales = clientes_profit_s.merge(clientes_mkwsp_s, 
                                                how ='inner',
                                                indicator=False)
    ...
    def datos_clientes_por_sinc_profit_mikrowisp(self):
        cod_clientes_profit_mikrowisp_por_sinc = self.__cod_clientes_profit_mikrowisp_por_sinc()
        clientes = self.clientes_profit()
        return clientes[clientes['co_cli'].isin(cod_clientes_profit_mikrowisp_por_sinc)][['co_cli', 
                                                                                          'cli_des', 
                                                                                          'direc1', 
                                                                                          'telefonos', 
                                                                                          'email'
    ...
    def __clientes_mikrowisp_aviso_user(self):
        data_aviso_user = self.data_mikrowisp.clientes_aviso_user()[['codigo_cliente', 'zona']]
        data_aviso_user.rename(columns={'codigo_cliente': 'co_cli', 'zona': 'campo3'}, 
                               inplace=True)
        data_aviso_user_s = data_aviso_user.sort_values(by=['co_cli'], 
                                                        ascending=[True])
        clientes_profit = self.clientes_profit()[['co_cli', 
                                                  'campo3']]
        clientes_profit_s = clientes_profit.sort_values(by=['co_cli'], 
        ...
        # Hace una combinación para identificar los datos comunes
        clientes_iguales = data_aviso_user_s.merge(clientes_profit_s, 
                                                how ='inner',
                                                indicator=False)
    ...
    def datos_clientes_nodo_por_sinc_mikrowisp_profit(self):
        ...
        clientes = self.data_mikrowisp.clientes_aviso_user() [['codigo_cliente','nombre', 'zona']]
        return clientes[clientes['codigo_cliente'].isin(
            cod_clientes_mikrowisp_profit_por_sinc)][['codigo_cliente', 'nombre', 'zona']]
    
    def sinc_datos_clientes_nodos(self):
        data = self.datos_clientes_nodo_por_sinc_mikrowisp_profit()
        conexion = ConexionBD(base_de_datos=self.data_base, 
                                host=os.getenv('HOST_PRODUCCION_PROFIT')) #  Crea un objeto conexión
