...
from accesos.datos import get_fecha_tasa_bcv_del_dia, get_monto_tasa_bcv_del_dia
from accesos.datos_profit import datos_profit
from accesos.transacciones import GestorTransacciones
from pandas import DataFrame, factorize, merge, to_datetime
...
class FacturacionMasiva:
    def __init__(self, data, host, data_base):
        ...
        self.conexion = ConexionBD(base_de_datos=data_base, host=host)
    ...
    def set_data_facturacion(self, data, **kwargs) -> DataFrame:
        ...
        data_["nro_doc"] = (
            str(doc_emit).zfill(6) if format_fact is True else int(doc_emit)
        )
        ...
        data_["indice_fact"] = data_.apply(
            lambda x: x["co_cli"] + str(x["enum"]), axis=1
        )
    ...
    def data_encab_facturacion_masiva(
        self, modulo, a_bs, num_doc, num_doc_ctrol, num_fact_format
    ) -> DataFrame:
        #  Agrupa el monto neto por articulo
        data_f = (
            self.data_facturacion.groupby(
                ["co_cli", "nro_doc", "nro_control", "fecha_fact", "descrip_encab_fact"]
            )["monto_base"]
            .sum()
            .reset_index()
        )
    ...
    def facturacion_masiva(self, modulo, a_bs, num_doc, num_doc_ctrol, num_fact_format):
        ...
        data = self.data_encab_facturacion_masiva(
            modulo, a_bs, num_doc, num_doc_ctrol, num_fact_format
        )
        ...
        data_iva = self.determinar_impuesto_por_factura(
            modulo, True, a_bs, num_doc, num_doc_ctrol, num_fact_format
        )
        data_con_iva = merge(
        ...
        for index, row in data_con_iva.iterrows():
            index += 1
            self.exe_sql_insert_encab_facturacion(
                row["nro_doc"],
    ...
    def procesar_det_facturacion(
        ...
        data_iva_temp = self.determinar_impuesto_por_factura(
            modulo, False, a_bs, num_doc, num_doc_ctrol, num_fact_format
        )
        ...
        for index, row in data_con_iva.iterrows():
            index += 1
            comentario = "{l1} \n {l2} \n {l3}".format(
                l1=row["comentario_l1"],
                l2=row["comentario_l2"],
                l3=row["comentario_l3"],
            ).replace("nan", "")

            self.exe_sql_insert_det_facturacion(
                row["correl"],
                row["nro_doc"],
                row["co_art"],
    ...
    def determinar_impuesto_por_factura(
        self, modulo, agrupado, a_bs, num_doc, num_doc_ctrol, num_fact_format
    ):
        ...
        merg1["iva"] = merg1.apply(
            lambda x: (
                round(x["monto_base"] * 16 / 100, ndigits=2)
                if x["tipo_imp"] == "1"
                else 0
            ),
            axis=1,
        ...
        merg1["p_iva"] = merg1.apply(
            lambda x: 16.0 if x["tipo_imp"] == "1" else 0, axis=1
        )
        if agrupado is True:
            # es necesario agrupar los encabezados de factura para totalizar la Base Imponible y el IVA
            data_agrupada = (
    ...
    def exe_sql_insert_encab_facturacion(
        self,
        id_doc,
        descrip,
        cod_cli,
    ...
    def exe_sql_insert_det_facturacion(
        self,
        linea,
        id_doc,
        co_art,
